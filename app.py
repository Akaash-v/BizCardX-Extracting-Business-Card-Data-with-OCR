...
def data_extract_from_mongodb():
    akku = pymongo.MongoClient(
        "mongodb+srv://root:root@cluster1.clroajt.mongodb.net/?retryWrites=true&w=majority")
    db = akku['bizcardx']
    col = db['image_data']
...
def image_decoded(image_name, user_name, password):
    akku = psycopg2.connect(host='localhost',
                        user='postgres',
                        password='root',
                        database='bizcardx')
    cursor = akku.cursor()
...
class data_extraction:
    ...
    name, designation = image_to_text.name_designation(formatted_data[0].split())
